...
static esp_err_t s_i2c_transaction_start(i2c_master_dev_handle_t i2c_dev, int xfer_timeout_ms)
{
    ...
    // A failed read can leave this flag set and make a later transaction's
    // ISR treat a probe/write operation as an RX transaction with no buffer.
...
I2C_MASTER_ISR_ATTR static void i2c_isr_clear_read_state(i2c_master_bus_t *i2c_master)
{
...
I2C_MASTER_ISR_ATTR static bool i2c_isr_receive_handler(i2c_master_bus_t *i2c_master)
{
    i2c_hal_context_t *hal = &i2c_master->base->hal;
    i2c_operation_t *i2c_operation;

    portENTER_CRITICAL_ISR(&i2c_master->base->spinlock);
    if (i2c_master->i2c_trans.ops == NULL) {
        goto invalid_read;
    }
    i2c_operation = &i2c_master->i2c_trans.ops[i2c_master->read_buf_pos];

    if (atomic_load(&i2c_master->status) == I2C_STATUS_READ) {
        if (i2c_master->rx_cnt > 0 && i2c_operation->data == NULL) {
            goto invalid_read;
        }
        if (i2c_master->rx_cnt > 0) {
        ...
        if (i2c_master->read_len_static > 0 && i2c_operation->data == NULL) {
            goto invalid_read;
        }
        if (i2c_master->read_len_static > 0) {
...
static void i2c_master_isr_handler_default(void *arg)
{
    ...
    if (transaction_failed) {
        i2c_isr_clear_read_state(i2c_master);
    } else if (i2c_master->contains_read == true) {
        if (int_mask & I2C_LL_INTR_MST_COMPLETE || int_mask & I2C_LL_INTR_END_DETECT) {
            (void)i2c_isr_receive_handler(i2c_master);
        }
    }
